...
export function drawLabels(
  ...
  context.strokeStyle = chartOptions.darkMode
    ? 'rgba(0,0,0,0.9)'
...
function ScatterChartThree(props) {
  ...
  useEffect(() => {
    ...
    scatterPlotRef.current.cameraCallback = (eventName, position, target) => {
      if (
        scatterPlotRef.current.interactionMode === 'PAN' &&
        trace.dimensions === 3
      ) {
        // repaint gallery charts with same embedding
        if (
          eventName === 'end' &&
          (previousCameraPosition.current.x != position.x ||
            previousCameraPosition.current.y != position.y ||
            previousCameraPosition.current.z != position.z)
        ) {
  ...
  function drawContext(context, chartSize, format) {
    ...
    for (let j = 0; j < npoints; j++) {
      ...
      if (is3d) {
        ...
        // Create size attenuation (if we're in 2D mode)
        ...
        const scale =
          zoom < 1
            ? 1 + outNorm * Math.atan(outSpeed * (zoom - 1))
