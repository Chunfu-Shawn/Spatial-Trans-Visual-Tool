...
export function createCategoryToStats(trace, selection) {
  ...
  for (const category in categoryToIndices) {
    ...
    if (!trace.continuous) {
      ...
      const mean = sum / n;
      categoryToStats[category] = {
        value: n === 0 ? undefined : (mean - trace.mean) / trace.stdev,
        n: n,
      };
    }
  }
...
class MetaEmbedding extends React.PureComponent {
  ...
  updateSvg = () => {
    ...
    if (
      ...
      childNodes[0].getAttribute('cirro-zoom') === 'true'
    ) {
      g = childNodes[0];
    }
    if (!g) {
      ...
      if (!!svgChildren && svgChildren.length > 0) {
        for (let i = svgChildren.length; i > 0; i--) {
          if (
            !nodeNamesKeep.has(svgChildren[svgChildren.length - i].nodeName)
          ) {
  ...
  onZoomOut = () => {
    select(this.props.trace.source).call(this.d3Zoom.scaleBy, 0.5);
  };
        ...
        if (categoryToStats) {
          ...
          if (stats) {
            ...
            if (this.props.trace.name !== '__count') {
              if (!this.props.trace.continuous) {
                tooltip +=
                  ', mode: ' +
                  stats.value +
                  (showFull ? ' (' + fullStats.value + ')' : '');
              } else {
                tooltip +=
                  ', z-score: ' +
                  stripTrailingZeros(numberFormat2f(stats.value)) +
                  (showFull
                    ? ' (' +
                      stripTrailingZeros(numberFormat2f(fullStats.value)) +
                      ')'
                    : '');
              }
            }
            tooltip +=
              ', # spots: ' +
              intFormat(stats.n) +
              (showFull ? ' / ' + intFormat(fullStats.n) : '');
          }
        }
      ...
      if (e.target.nodeName === 'path') {
        ...
        if (indices && indices.length > 0) {
          this.props.onSelected({
            ...
            value: {
              basis: this.props.trace.embedding,
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (this.props.trace.source !== prevProps.trace.source) {
      ...
      this.props.chartSize.width !== prevProps.chartSize.width ||
      this.props.chartSize.height !== prevProps.chartSize.height
    ) {
      const svg = this.props.trace.source;
      svg.setAttribute('width', this.props.chartSize.width);
      svg.setAttribute('height', this.props.chartSize.height);
