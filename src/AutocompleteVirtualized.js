...
function getTextMatch(text, inputValue) {
  ...
  if (inputValue !== '') {
    ...
    if (index !== -1) {
      // bold the matching text when searching
...
function renderRow(props) {
  ...
  if (dataSet.hasOwnProperty('group')) {
    ...
    const textMatch =
      dataSet.inputValue != null
        ? getTextMatch(dataSet.group, dataSet.inputValue)
        : null;
    if (textMatch) {
      return (
        <Typography component="li" {...dataSet[0]} style={inlineStyle}>
          ...
          {dataSet.selectGroup && (
            <Link
              href="#"
              onClick={(e) => dataSet.selectGroup(e, dataSet.group)}
            >
              All
            </Link>
          )}
    ...
    return (
      <Typography component="li" {...dataSet[0]} style={inlineStyle}>
        {dataSet.group}{' '}
        {dataSet.selectGroup && (
          <Link href="#" onClick={(e) => dataSet.selectGroup(e, dataSet.group)}>
            All
          </Link>
        )}
      </Typography>
...
export default function AutocompleteVirtualized(props) {
  ...
  function enterTokens(event, tokens) {
    const results = value;
    tokens = tokens
      .map((token) => token.toLowerCase().trim().replace(/["']/g, ''))
      .filter((token) => token !== '');
    if (tokens.length > 0) {
  ...
  const onChipClick = props.onChipClick
    ? (event, option) => {
  ...
  if (getOptionSelected == null) {
    getOptionSelected = props.groupBy
      ? (option, value) =>
  ...
  if (getOptionLabel == null) {
    getOptionLabel = props.groupBy
      ? (option) => option.text
      : (option) => option;
  }

  const selectGroup = props.selectGroup
    ? (event, group) => {
